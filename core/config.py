"""Configuration management for MCP Wan server."""
...
class Settings:
    ...
    default_resolution: str = field(
        default_factory=lambda: os.getenv("WAN_DEFAULT_RESOLUTION", "720P")
    )

    # Request Configuration
    request_timeout: float = field(
        default_factory=lambda: float(os.getenv("WAN_REQUEST_TIMEOUT", "1800"))
    )

    # Server Configuration
    server_name: str = field(default_factory=lambda: os.getenv("MCP_SERVER_NAME", "wan"))
